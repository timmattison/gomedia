...
func (entry *SampleEntry) Decode(r io.Reader) (offset int, err error) {
	...
	offset = 6
	entry.dataReferenceIndex = binary.BigEndian.Uint16(buf[offset:])
	offset += 2
	return
}
...
func (entry *SampleEntry) Encode() (int, []byte) {
	...
	offset += 6
	binary.BigEndian.PutUint16(buf[offset:], entry.dataReferenceIndex)
	offset += 2
	return offset, buf
}
...
func (entry *VisualSampleEntry) Decode(r io.Reader) (offset int, err error) {
	...
	offset += 8
	entry.frameCount = binary.BigEndian.Uint16(buf[offset:])
	offset += 2
...
func (entry *VisualSampleEntry) Encode() (int, []byte) {
	...
	offset += 8
	binary.BigEndian.PutUint16(buf[offset:], entry.frameCount)
	offset += 2
...
const (
	SampleAudio SampleEntryType = iota
	SampleVideo
)
...
func (stsd *SampleDescriptionBox) Decode(r io.Reader) (offset int, err error) {
	...
	stsd.entryCount = binary.BigEndian.Uint32(buf)
	return
}

func (entry *SampleDescriptionBox) Encode() (int, []byte) {
	offset, buf := entry.box.Encode()
	binary.BigEndian.PutUint32(buf[offset:], entry.entryCount)
	offset += 4
	return offset, buf
}

func makeStsd(track *mp4track, handlerType HandlerType) []byte {

	var avbox []byte
	var extraData []byte
	if len(track.extraData) == 0 {
		if track.cid == Mp4CodecAac || track.cid == Mp4CodecH264 || track.cid == Mp4CodecH265 {
			if track.extra == nil {
	...
	if track.cid == Mp4CodecH264 {
		avbox = makeAvcCBox(extraData)
	} else if track.cid == Mp4CodecH265 {
		avbox = makeHvcCBox(extraData)
	} else if track.cid == Mp4CodecAac || track.cid == Mp4CodecMp2 || track.cid == Mp4CodecMp3 {
		avbox = makeEsdsBox(track.trackId, track.cid, extraData)
	} else if track.cid == Mp4CodecOpus {
		avbox = makeOpusSpecificBox(extraData)
	}

	var se []byte
	var offset int
	if handlerType.equal(vide) {
	...
	} else if handlerType.equal(soun) {
...
func makeEsdsBox(tid uint32, cid Mp4CodecType, extraData []byte) []byte {
