...
func (mvhd *MovieHeaderBox) Decode(r io.Reader) (offset int, err error) {
	...
	n := 0
	if mvhd.Box.Version == 1 {
		mvhd.CreationTime = binary.BigEndian.Uint64(buf[n:])
		n += 8
		mvhd.ModificationTime = binary.BigEndian.Uint64(buf[n:])
		n += 8
		...
		mvhd.CreationTime = uint64(binary.BigEndian.Uint32(buf[n:]))
		n += 4
		mvhd.ModificationTime = uint64(binary.BigEndian.Uint32(buf[n:]))
		n += 4
		mvhd.Timescale = binary.BigEndian.Uint32(buf[n:])
		n += 4
	...
	for i := 0; i < 6; i++ {
		mvhd.PreDefined[i] = binary.BigEndian.Uint32(buf[n:])
		n += 4
	}
	mvhd.NextTrackId = binary.BigEndian.Uint32(buf[n:])
...
func (mvhd *MovieHeaderBox) Encode() (int, []byte) {
	...
	if mvhd.Box.Version == 1 {
		binary.BigEndian.PutUint64(buf[offset:], mvhd.CreationTime)
		offset += 8
		binary.BigEndian.PutUint64(buf[offset:], mvhd.ModificationTime)
		offset += 8
		...
		binary.BigEndian.PutUint32(buf[offset:], uint32(mvhd.CreationTime))
		offset += 4
		binary.BigEndian.PutUint32(buf[offset:], uint32(mvhd.ModificationTime))
		offset += 4
	...
	offset += 24
	binary.BigEndian.PutUint32(buf[offset:], mvhd.NextTrackId)
	return offset + 2, buf
}

func makeMvhdBox(trackid uint32, duration uint32) []byte {
	mvhd := NewMovieHeaderBox()
	mvhd.NextTrackId = trackid
...
func decodeMvhd(demuxer *MovDemuxer) (err error) {
	...
	demuxer.mp4Info.CreateTime = mvhd.CreationTime
	demuxer.mp4Info.ModifyTime = mvhd.ModificationTime
	return
}
