...
const (
	...
	TrFlagDataFirstSampleFlags      uint32 = 0x000004
	TrFlagDataSampleDuration        uint32 = 0x000100
	TrFlagDataSampleSize            uint32 = 0x000200
	TrFlagDataSampleFlags           uint32 = 0x000400
	TrFlagDataSampleCompositionTime uint32 = 0x000800
)
...
func (trun *TrackRunBox) Size() uint64 {
	...
	if trunFlags&uint32(TrFlagDataOffset) > 0 {
		n += 4
	}
	if trunFlags&uint32(TrFlagDataFirstSampleFlags) > 0 {
		n += 4
	}
	if trunFlags&uint32(TrFlagDataSampleDuration) > 0 {
		n += 4 * uint64(trun.SampleCount)
	}
	if trunFlags&uint32(TrFlagDataSampleSize) > 0 {
		n += 4 * uint64(trun.SampleCount)
	}
	if trunFlags&uint32(TrFlagDataSampleFlags) > 0 {
		n += 4 * uint64(trun.SampleCount)
	}
	if trunFlags&uint32(TrFlagDataSampleCompositionTime) > 0 {
		n += 4 * uint64(trun.SampleCount)
	}
	return n
}
...
func makeTrunBox(track *mp4track, start, end int, moofSize uint64) []byte {
	...
	for j := start; j < end; j++ {
		...
		if track.samplelist[j].pts != track.samplelist[j].dts {
			flag |= TrFlagDataSampleCompositionTime
		}
	}
	...
	trun.FirstSampleFlags = MovFragSampleFlagDependsNo
	trun.EntryList = new(movtrun)
	for i := start; i < end; i++ {
