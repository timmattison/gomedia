...
func (pack *TsPacker) Pack(data []byte, timestamp uint32) error {
	for len(data) > 0 {
		...
		pkg.Header.Marker = 0
		if len(data) > pack.mtu-RtpFixHeadLen {
			pkg.Payload = make([]byte, pack.mtu-RtpFixHeadLen)
			copy(pkg.Payload, data[:pack.mtu-RtpFixHeadLen])
			data = data[pack.mtu-RtpFixHeadLen:]
		} else {
